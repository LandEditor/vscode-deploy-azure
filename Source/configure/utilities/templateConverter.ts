...
export function convertExpression(expression: string): string {
	...
	if (
		...
		expression.indexOf(" ") < expression.indexOf(closingHelperFunc)
	) {
	...
	if (
		expression.substr(
			...
			openingHelperFunc.length + 1 + helperName.length,
		) ===
		openingHelperFunc + "/" + helperName
	) {
		parts.push(
			expression.substring(
				expression.indexOf(closingHelperFunc) +
