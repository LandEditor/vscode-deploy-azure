...
export class LocalGitHubWorkflowConfigurer implements Configurer {
			...
			if (githubOrganizations && githubOrganizations.length > 0) {
				const selectedOrganization =
					await this.controlProvider.showQuickPick(
						...
						githubOrganizations.map((x) => {
							return { label: x.login };
						}),
				...
				try {
					const newGitHubRepo = (await generateGitHubRepository(
	...
	public async createPreRequisites(
		...
		if (
			...
			inputs.targetResource.resource.type.toLowerCase() ===
				TargetResourceType.WebApp.toLowerCase()
		) {
			let azureConnectionSecret: string =
	...
	public async createAsset(
		...
		switch (type) {
			...
			default:
				throw new Error(
					utils.format(
						Messages.assetOfTypeNotSupportedForGitHub,
						type,
					),
				);
		}
	...
	public async getPathToManifestFile(
		...
		let manifestsDirectoryPath: string = path.join(
							...
							try {
								...
								return await localGitRepoHelper.commitAndPushPipelineFile(
									...
									extensionVariables.enableGitHubWorkflow
										? Messages.addGitHubWorkflowYmlFile
	...
	public async executePostPipelineCreationSteps(
		...
		try {
			if (
				inputs.targetResource &&
				inputs.targetResource.resource &&
				inputs.targetResource.resource.type ===
				...
				let aksResource: GenericResource =
				...
				await azureResourceClient.updateCdSetupResourceTag(
					...
					workflowFileName,
					inputs.sourceRepository.commitId,
					inputs.pipelineConfiguration.params["namespace"],
					ApiVersions.get(TargetResourceType.AKS),
				);
			}
		} catch (error) {
			telemetryHelper.logError(
	...
	public async browseQueuedPipeline(): Promise<void> {
		vscode.window
			.showInformationMessage(
				Messages.githubWorkflowSetupSuccessfully,
	...
	protected async getAzureSPNSecret(
		inputs: WizardInputs,
		scope?: string,
	): Promise<string> {
