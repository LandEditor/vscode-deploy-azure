...
import {
	AppServiceClient,
	VSTSDeploymentMessage,
} from "../clients/azure/appServiceClient";
import { AzureResourceClient } from "../clients/azure/azureResourceClient";
import { AzureDevOpsClient } from "../clients/devOps/azureDevOpsClient";
import { UniqueResourceNameSuffix } from "../configure";
import {
	generateDevOpsOrganizationName,
	generateDevOpsProjectName,
...
export class AzurePipelineConfigurer implements Configurer {
	...
	public async getInputs(inputs: WizardInputs): Promise<void> {
		try {
			...
			if (
				...
				let devOpsOrganizations =
					await this.azureDevOpsClient.listOrganizations();
				...
				if (devOpsOrganizations && devOpsOrganizations.length > 0) {
					let selectedOrganization =
						await this.controlProvider.showQuickPick(
							...
							devOpsOrganizations.map((x) => {
								return { label: x.accountName };
							}),
					...
					let selectedProject =
						await this.controlProvider.showQuickPick(
							...
							this.azureDevOpsClient
								.listProjects(inputs.organizationName)
								.then((projects) =>
									projects.map((x) => {
					...
					if (validationErrorMessage) {
						inputs.organizationName =
							await this.controlProvider.showInputBox(
								...
								{
									placeHolder:
										Messages.enterAzureDevOpsOrganizationName,
									validateInput: (organizationName) =>
										this.azureDevOpsClient.validateOrganizationName(
											organizationName,
										),
								},
							);
			...
			telemetryHelper.setTelemetry(
				TelemetryKeys.NewOrganization,
				inputs.isNewOrganization.toString(),
			);
		} catch (error) {
			if (
				(error.typeKey as string).toUpperCase() ===
				constants.ExceptionType.UnauthorizedRequestException
			) {
				throw new WhiteListedError(
					(error.message as string).concat(
						Messages.AdoDifferentTenantError,
					),
					error,
				);
			}
			...
			await vscode.window.withProgress(
				...
				async () => {
					try {
						...
						await this.azureDevOpsClient.createProject(
							inputs.organizationName,
							inputs.project.name,
						);
						inputs.project.id =
				...
				await vscode.window.withProgress(
					{
						location: vscode.ProgressLocation.Notification,
						title: utils.format(
							Messages.creatingAzureServiceConnection,
	...
	public async createAsset(
		...
		switch (type) {
			...
			default:
				throw new Error(
					utils.format(
						Messages.assetOfTypeNotSupportedForAzurePipelines,
						type,
					),
				);
		}
	...
	public async getPathToPipelineFile(
		inputs: WizardInputs,
	...
	public async getPathToManifestFile(
		inputs: WizardInputs,
		localGitRepoHelper: LocalGitRepoHelper,
		fileName: string,
	): Promise<string> {
	...
	public async checkInPipelineFilesToRepository(
		...
		while (!inputs.sourceRepository.commitId) {
			...
			if (
				...
				inputs.sourceRepository.commitId =
					await vscode.window.withProgress(
						{
							location: vscode.ProgressLocation.Notification,
						...
						async () => {
							try {
								...
								if (initializeGitRepository) {
									await localGitRepoHelper.initializeGitRepository(
										inputs.sourceRepository.remoteName,
										inputs.sourceRepository.remoteUrl,
								...
								return await localGitRepoHelper.commitAndPushPipelineFile(
									filesToCommit,
									inputs.sourceRepository,
									Messages.addAzurePipelinesYmlFile,
								);
							} catch (error) {
								telemetryHelper.logError(
									Layer,
	...
	public async createAndQueuePipeline(inputs: WizardInputs): Promise<string> {
		this.queuedPipeline = await vscode.window.withProgress<Build>(
			{
				location: vscode.ProgressLocation.Notification,
				title: Messages.configuringPipelineAndDeployment,
			},
	...
	public async executePostPipelineCreationSteps(
		inputs: WizardInputs,
		azureResourceClient: AzureResourceClient,
	): Promise<void> {
		if (
			...
			try {
				// update SCM type
				let targetResource: GenericResource =
					AzurePipelineConfigurer.getTargetResource(inputs);

				let updateScmPromise = (
					azureResourceClient as AppServiceClient
				).updateScmType(targetResource.id);

				let buildDefinitionUrl =
					this.azureDevOpsClient.getOldFormatBuildDefinitionUrl(
						inputs.organizationName,
						inputs.project.id,
				...
				Q.all([
					...
					updateDeploymentLogPromise,
				]).then(() => {
					telemetryHelper.setTelemetry(
						TelemetryKeys.UpdatedWebAppMetadata,
						"true",
					);
				});
			} catch (error) {
				telemetryHelper.logError(
					Layer,
					TracePoints.PostDeploymentActionFailed,
					error,
				);
			}
		}
		...
		vscode.window
			...
			.then((action: string) => {
				if (
					action &&
					action.toLowerCase() ===
						Messages.browsePipeline.toLowerCase()
				) {
					telemetryHelper.setTelemetry(
	...
	private static getTargetResource(inputs: WizardInputs): GenericResource {
		...
		if (!targetResource) {
			...
			if (
				...
				throw new Error(
					utils.format(
						Messages.couldNotFindTargetResourceValueInParams,
						inputs.pipelineConfiguration.template.targetType,
					),
				);
			}
	...
	private async createAzurePublishProfileEndpoint(
	...
	private async createAzureSPNServiceEndpoint(
		serviceConnectionHelper: ServiceConnectionHelper,
		serviceConnectionName: string,
		inputs: WizardInputs,
	): Promise<string> {
