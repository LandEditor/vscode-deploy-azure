...
export class RemoteGitHubWorkflowConfigurer extends LocalGitHubWorkflowConfigurer {
		...
		if (!!assets && assets.length > 0) {
			for (let asset of assets) {
				if (asset.stage === stage) {
					asset = MustacheHelper.renderObject(
						asset,
						this.mustacheContext,
					);
	...
	public async getPipelineFilesToCommit(
		inputs: WizardInputs,
	): Promise<string[]> {
			...
			switch (asset.type) {
					...
					this.assets[asset.id] = await vscode.window.withProgress(
						...
						async () => {
							const inputDescriptor =
								this.template.parameters.inputs.find(
									(value) =>
										value.type ===
				...
				case "SetGHSecret":
					...
					await vscode.window.withProgress(
						...
						async () => {
							await this.githubClient.createOrUpdateGithubSecret(
								secretKey,
								secretValue,
							);
						},
							...
							await vscode.window.withProgress(
								...
								async () => {
									try {
										...
										let appServiceClient =
											new AppServiceClient(
												this.azureSession.credentials,
												this.azureSession.environment,
				...
				default:
					throw new Error(
						utils.format(
							Messages.assetOfTypeNotSupported,
							asset.type,
						),
					);
			}
	...
	private async getWorkflowFile(inputs: WizardInputs): Promise<File> {
		let pipelineDefinition = MustacheHelper.renderObject(
		...
		await vscode.window.withProgress(
			...
			async () => {
				workflowFileContent = await this.getTemplateFile(
					pipelineDefinition.templateFile,
				);
			},
		);
	...
	private async getTemplateFile(fileName: string): Promise<string> {
		try {
			let result = await this.templateServiceClient.getTemplateFile(
				this.template.id,
				fileName,
			);
