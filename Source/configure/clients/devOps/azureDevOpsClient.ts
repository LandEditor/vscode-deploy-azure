...
export class AzureDevOpsClient {
	...
	public async sendRequest(
		urlBasedRequestPrepareOptions: UrlBasedRequestPrepareOptions,
	): Promise<any> {
		if (urlBasedRequestPrepareOptions.headers) {
			urlBasedRequestPrepareOptions.headers["X-TFS-Session"] =
				telemetryHelper.getJourneyId();
		} else {
			urlBasedRequestPrepareOptions.headers = {
	...
	public async createBuildDefinition(
		organizationName: string,
	...
	public async validateOrganizationName(
		...
		if (
			!organizationName ||
			/^\\s/.test(organizationName) ||
			/\\s$/.test(organizationName) ||
			organizationName.indexOf("-") === 0 ||
			!accountNameRegex.test(organizationName)
		) {
			deferred.resolve(Messages.organizationNameStaticValidationMessage);
		} else if (ReservedHostNames.indexOf(organizationName) >= 0) {
			deferred.resolve(
				util.format(
					Messages.organizationNameReservedMessage,
					organizationName,
				),
			);
		} else {
			...
			this.sendRequest(<UrlBasedRequestPrepareOptions>{
				...
				.then((response) => {
					if (
						response.name === organizationName &&
	...
	public async getProjectIdFromName(
		organizationName: string,
		projectName: string,
	): Promise<string> {
	...
	public async getOrganizationIdFromName(organizationName: string) {
		let organization = (await this.listOrgPromise).find((org) => {
			return (
				org.accountName.toLowerCase() === organizationName.toLowerCase()
			);
		});
	...
	public getOldFormatBuildDefinitionUrl(
		accountName: string,
		projectName: string,
	...
	private getUserData(): Promise<any> {
		return this.getConnectionData().catch(() => {
			return this.createUserProfile().then(() => {
	...
	private async monitorOperationStatus(operationUrl: string): Promise<void> {
		...
		while (retryCount < 30) {
			...
			if (result === "succeeded") {
				...
				throw new Error(
					util.format(
						Messages.failedToCreateAzureDevOpsProject,
						operationResult.detailedMessage,
					),
				);
			} else {
				retryCount++;
		...
		throw new Error(
			util.format(
				Messages.failedToCreateAzureDevOpsProject,
				(operationResult && operationResult.detailedMessage) ||
					Messages.operationTimedOut,
			),
		);
	}
