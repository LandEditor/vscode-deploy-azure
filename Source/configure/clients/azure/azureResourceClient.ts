...
export class AzureResourceClient {
	...
	constructor(credentials: ServiceClientCredentials, subscriptionId: string) {
		this.azureRmClient =
			new ResourceManagementClient.ResourceManagementClient(
				credentials,
				subscriptionId,
			);
	}
	...
	public static validateTargetResourceType(resource: GenericResource): void {
		...
		switch (resource.type.toLowerCase()) {
			case TargetResourceType.WebApp.toLowerCase():
				switch (resource.kind ? resource.kind.toLowerCase() : "") {
					...
					default:
						throw new Error(
							utils.format(
								Messages.appKindIsNotSupported,
								resource.kind,
							),
						);
				}
			...
			default:
				throw new Error(
					utils.format(
						Messages.resourceTypeIsNotSupported,
						resource.type,
					),
				);
		}
	...
	public async getResourceList(
		resourceType: string,
		followNextLink: boolean = true,
	): Promise<ResourceListResult> {
		let resourceListResult: ResourceListResult =
			await this.azureRmClient.resources.list({
	...
	public async updateCdSetupResourceTag(
	...
	private ComputeDeploymentResourceTags(
		...
		for (let tagName in resourceTags) {
			...
			if (tagName.startsWith(DevopsInfoTagHeader)) {
				...
				// check if resource tags can be stored in tag Value field
					...
					resourceTags[tagName].length + deploymentData.length <
					MaxTagValueLength
				) {
					storageColumn = false;
		...
		if (storageColumn) {
			//Store resource tag in key field
			newTagKey += deploymentData + ";";
		} else {
			//Store resource tag in value field
...
export let ApiVersions: Map<TargetResourceType, string> = new Map<
	TargetResourceType,
	string
>();
