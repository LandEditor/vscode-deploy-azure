...
export class GithubClient {
	...
	public async createOrUpdateGithubSecret(
		secretName: string,
		body: string,
	): Promise<void> {
	...
	public async createGithubRepo(
		...
		return this._sendRequest(<UrlBasedRequestPrepareOptions>{
			...
			body: {
				name: repoName,
				description:
					"Repo created from VScode extension 'Deploy to Azure'",
		...
		if (!this.listOrgPromise || forceRefresh) {
			this.listOrgPromise = Promise.all([
				...
				return (organizations as GitHubOrganization[])
					...
					.sort((org1, org2) =>
						stringCompareFunction(org1.login, org2.login),
					);
			});
		}
	...
	public async _getGitHubSecretKey(): Promise<IGitHubSecretKey> {
		const request = <UrlBasedRequestPrepareOptions>{
			url:
				GitHubProvider.getFormattedGitHubApiUrlBase(this.url) +
