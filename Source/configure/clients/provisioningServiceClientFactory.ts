...
import {
	...
	ServiceFramework,
} from "../helper/remoteServiceUrlHelper";
...
export class ProvisioningServiceClientFactory {
	public static async getClient(): Promise<IProvisioningServiceClient> {
		...
		const defaultHeaders: { [propertyName: string]: string } = {
			"Content-Type": "application/json",
		};

		const serviceDefinition =
			await RemoteServiceUrlHelper.getProvisioningServiceDefinition();
		...
		if (serviceDefinition.serviceFramework === ServiceFramework.Vssf) {
			...
			this.client = new ProvisioningServiceClient(
				serviceDefinition,
				defaultHeaders,
				new TokenCredentials(this.githubPatToken, "token"),
			);
		}
