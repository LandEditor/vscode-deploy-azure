	...
	await telemetryHelper.executeFunctionWithTimeTelemetry(async () => {
		try {
			if (!!node && !!node.fullId) {
				...
				throw new Error(
					Messages.didNotRecieveAzureResourceNodeToProcess,
				);
			}
		} catch (error) {
...
async function browsePipelineInternal(
	resourceId: string,
	...
	if (scmType === ScmType.VSTSRM.toLowerCase()) {
		...
		scmType === ScmType.GITHUBACTION.toLowerCase() &&
...
async function browseAzurePipeline(
	resourceId: string,
	appServiceClient: AppServiceClient,
): Promise<void> {
	try {
...
async function browseGitHubWorkflow(
	resourceId: string,
	appServiceClient: AppServiceClient,
): Promise<void> {
	...
	if (
		!!webAppSourceControl &&
		!!webAppSourceControl.properties &&
