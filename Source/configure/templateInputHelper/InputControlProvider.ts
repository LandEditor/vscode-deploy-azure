...
export class InputControlProvider {
	...
	constructor(
		...
		pipelineTemplate: ExtendedPipelineTemplate,
	...
	public async getAllPipelineTemplateInputs(): Promise<{
		[key: string]: any;
	}> {
		...
		for (let inputControl of this._inputControlsMap.values()) {
			...
			if (
				...
				await this.repoAnalysisSettingInputProvider.setInputControlValueFromRepoAnalysisResult(
					inputControl,
				);
			} else {
	...
	private _setInputControlDataSourceInputs(inputControl: InputControl): void {
		...
		if (!!inputDes.dataSourceId) {
			...
			if (inputControl.dataSource) {
				var dependentInputControlArray = this._getInputDependencyArray(
					inputControl,
				...
				throw new Error(
					`Data source ${inputDes.dataSourceId} specified for input ${inputDes.id} is not present in pipeline template ${this._pipelineTemplate.id}`,
				);
			}
		}
	}
			...
			inputDes.dynamicValidations.forEach(
				(validation: InputDynamicValidation) => {
					var validationDataSource =
						DataSourceUtility.getDataSourceById(
							this._pipelineTemplate.parameters.dataSources,
					...
					if (validationDataSource) {
						...
						let dynamicValidationRequiredInputIds =
						...
						dynamicValidationRequiredInputIds =
							dynamicValidationRequiredInputIds.concat(
								DataSourceUtility.getDependentInputIdList(
									validationDataSource.resultSelector,
								),
							);
	...
	private _computeMustacheValue(
		mustacheExpression: string,
		dependentInputControlArray: InputControl[],
		dependentClientInputMap: StringMap<any>,
	): any {
		...
		if (
			...
			!dependentInputValues
		) {
			return "";
		} else {
			return MustacheHelper.renderObject(mustacheExpression, {
				inputs: dependentInputValues,
				client: dependentClientInputMap,
			});
		}
	}

	private _getInputControlValue(inputDes: ExtendedInputDescriptor): string {
		if (!!this._context && !!this._context[inputDes.id]) {
			return this._context[inputDes.id];
		} else {
			return !inputDes.defaultValue ||
				InputControlUtility.doesExpressionContainsDependency(
	...
	private _getClientDependencyMap(
		...
		for (var dependencyExpression of dependencyExpressionArray) {
			if (dependencyExpression) {
				dependentClientInputs = dependentClientInputs.concat(
					InputControlUtility.getDependentClientIdList(
		...
		var uniqueDependentClientInputs = dependentClientInputs.filter(
			function (item, pos) {
		...
		for (var clientInput of uniqueDependentClientInputs) {
			...
			if (dependentInputControl !== undefined) {
				...
				throw new Error(
					`Dependent client input ${clientInput} specified is not present in client context.`,
				);
			}
		}
	...
	private _getInputDependencyArray(
		inputControl: InputControl,
		dependencyExpressionArray: string[],
		...
		for (var dependencyExpression of dependencyExpressionArray) {
			if (dependencyExpression) {
				dependentInputIds = dependentInputIds.concat(
					InputControlUtility.getDependentInputIdList(
						dependencyExpression,
					),
				);
			}
		...
		if (
			!allowSelfDependency &&
			dependentInputIds.indexOf(inputControl.getInputControlId()) >= 0
		) {
			throw new Error(
				`Input ${inputControl.getInputControlId()} has dependency on its own in pipeline template ${this._pipelineTemplate.id}.`,
			);
		}

		var uniqueDependentInputIds = dependentInputIds.filter(
			function (item, pos) {
		...
		for (var inputId of uniqueDependentInputIds) {
			...
			if (dependentInputControl) {
				...
				throw new Error(
					`Dependent input ${inputId} specified for input ${inputControl.getInputControlId()} is not present in pipeline template ${this._pipelineTemplate.id}`,
				);
			}
		}
	...
	private _getInputParameterValueIfAllSet(
		dependentInputControlArray: InputControl[],
	...
	private _setInputControlVisibility(inputControl: InputControl): void {
		...
		if (
			...
			visibilityRule.predicateRules.length >= 0
		) {
			var requiredInputControls: InputControl[] = [];

			visibilityRule.predicateRules.forEach(
				(predicateRule: IPredicate) => {
					try {
						requiredInputControls = requiredInputControls.concat([
							this._inputControlsMap.get(predicateRule.inputName),
						]);
					} catch (exception) {
						throw new Error(
							"Input defined in visiblity rule doesn't exist",
						);
					}
				},
			);
