...
export class InputControl {
	...
	constructor(
		inputDescriptor: ExtendedInputDescriptor,
		value: any,
	...
	public async setInputControlValue(): Promise<any> {
		...
		if (!!this.dataSource) {
			...
			if (
				...
				this.value = await this.dataSource.evaluateDataSources(
					dependentInputs,
					this.azureSession,
				);
				...
				if (errorMessage) {
					...
					this.value = await this.controlProvider.showInputBox(
						this.getInputControlId(),
						{
							...
							validateInput: (value) =>
								this.triggerControlValueValidations(value),
						},
					);
				}
			} else if (this.controlType === ControlType.QuickPick) {
				let listItems: Array<QuickPickItemWithData> =
					await vscode.window.withProgress(
						{
							location: vscode.ProgressLocation.Notification,
							title: utils.format(
								Messages.fetchingInputMessage,
								this.inputDescriptor.name,
							),
						},
						() =>
							this.dataSource.evaluateDataSources(
								dependentInputs,
								this.azureSession,
							),
					);
	...
	public getPropertyValue(
		...
		return InputControl.getInputDescriptorProperty(
	...
	public static getInputDescriptorProperty(
		input: ExtendedInputDescriptor,
		propertyName: string,
		inputs?: StringMap<any>,
	): any {
	...
	public setValidationDataSources(
		dataSourceToInputsMap: Map<DataSource, InputControl[]>,
	) {
		this.validationDataSourceToInputsMap = dataSourceToInputsMap;
	}

	public async triggerControlValueValidations(
		value: string,
	): Promise<string> {
		...
		this.inputDescriptor.dynamicValidations.forEach(
			(validation: InputDynamicValidation) => {
				validationPromises.push(
					this.evaluateDynamicValidation(validation, value),
				);
			},
		);
	...
	private getStaticValidationsResult(value: string): string {
		...
		if (this.inputDescriptor.staticValidation) {
			if (
				...
				let result = StaticValidator.validateLength(
					value,
					this.inputDescriptor.staticValidation.minLength,
					this.inputDescriptor.staticValidation.maxLength,
				);
			...
			if (!!this.inputDescriptor.staticValidation.pattern) {
				...
				let result = StaticValidator.validateRegex(
					value,
					regexPattern,
			...
			if (
				...
				let result = StaticValidator.validateNumberValue(
					value,
					this.inputDescriptor.staticValidation.minValue,
					this.inputDescriptor.staticValidation.maxValue,
				);
	...
	private async evaluateDynamicValidation(
		validation: InputDynamicValidation,
		value: string,
	): Promise<string> {
		...
		this.validationDataSourceToInputsMap
			...
			.forEach((descriptor) => {
				...
				requiredInputsValueMap[descriptor.getInputControlId()] =
					descriptor.getValue();
			});
		...
		if (allRequiredInputValuesAvailable) {
			return DataSourceUtility.evaluateDataSource(
				...
				.catch((error) => {
					if (
						!(
							requiredDataSource.resultTemplate &&
							requiredDataSource.resultTemplate.toLowerCase() ===
								"false"
						)
					) {
						telemetryHelper.logError(
							Layer,
							TracePoints.EvaluateDynamicValidation,
							error,
						);
					}
					return null;
