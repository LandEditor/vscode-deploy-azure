...
export class InputControlUtility {
	public static doesExpressionContainsDependency(
		expression: string,
	): boolean {
		if (!expression) {
			return false;
		...
		return /{{{inputs\.\w+}}}|{{{system\.\w+}}}|{{{client\.\w+}}}/g.test(
			expression,
		);
	}

	public static getDependentInputIdList(expression: string): string[] {
		return this._fetchIdsInExpressionByType(
			expression,
	...
	private static _fetchIdsInExpressionByType(
		expression: string,
