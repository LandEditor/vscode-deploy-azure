...
export class DataSourceExpression {
	...
	public getInputDependencyArray(): string[] {
		if (this.operator === Operator.UNDEFINED) {
			var dataSource = DataSourceUtility.getDataSourceById(
				this._dataSources,
				this.value,
			);
			return [
	...
	public async evaluateDataSources(
		...
		if (this.leftChild) {
			dataPromises.push(
				this.leftChild.evaluateDataSources(inputs, azureSession),
			);
		}

		if (this.rightChild) {
			dataPromises.push(
				this.rightChild.evaluateDataSources(inputs, azureSession),
			);
		}
		...
		return Promise.all(dataPromises).then((result: any[]) => {
			...
			if (Array.isArray(result[0]) && Array.isArray(result[1])) {
				return DataSourceExpression._applyOperator(
					this.operator,
					result[0],
					result[1],
				);
	...
	private constructor(
		operator: Operator,
		leftChild: DataSourceExpression,
		rightChild: DataSourceExpression,
		value: string,
		dataSources: DataSource[],
	) {
	...
	private static _applyOperator(
		operator: Operator,
		array1: Array<{ text: string; value: any; group?: string }>,
		array2: Array<{ text: string; value: any; group?: string }>,
	): Array<{ text: string; value: any; group?: string }> {
		switch (operator) {
			case Operator.INTERSECT:
				return array1.filter((item1) => {
					return array2.some((item2) => {
						return (
							item2.value.toLowerCase() ===
								item1.value.toLowerCase() &&
	...
	private static _validateDataSourceId(dataSourceId: string): void {
		...
		if (dataSourceId.startsWith("(") || dataSourceId.endsWith(")")) {
			error =
				"Invalid DataSourceId. It should not start or end with braces";
		}
