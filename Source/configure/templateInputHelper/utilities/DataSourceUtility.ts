...
export class DataSourceUtility {
	public static getDependentInputIdList(
	...
	public static async evaluateDataSource(
		...
		return amrClient
			...
			.then((response: any) => {
				return this.evaluateDataSourceResponse(
					dataSource,
					response,
					view,
				);
	...
	private static evaluateDataSourceResponse(
		dataSource: DataSource,
		response: any,
		view: { inputs: StringMap<any> },
	): any {
		if (!!dataSource.resultSelector) {
			var resultSelector = MustacheHelper.render(
		...
		if (Array.isArray(response)) {
			...
			if (!!dataSource.resultTemplate) {
				response = MustacheHelper.render(dataSource.resultTemplate, {
					...view,
					...response,
				});
			}
