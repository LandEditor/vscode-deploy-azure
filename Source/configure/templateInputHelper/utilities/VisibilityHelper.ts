...
export class VisibilityHelper {
	...
	public static evaluateVisibility(
		visibilityRule: IVisibilityRule,
		dependentInputs: InputControl[],
	): boolean {
		...
		for (
			...
			let dependentInput: InputControl = dependentInputs.find(
				(dependentInput: InputControl) => {
					return (
						dependentInput.getInputControlId() ===
						predicateRule.inputName
					);
				},
			);
			...
			if (dependentInput) {
				...
				if (!isInputVisible) {
					...
					let predicateResult = this._getPredicateResult(
						predicateRule,
	...
	private static _getPredicateResult(
		rule: IPredicate,
		valueToCheck: string,
	): boolean {
		let returnValue: boolean = false;

		let valueToCheckLowerCase = valueToCheck
			? valueToCheck.toString().toLowerCase()
			: valueToCheck;

		if (rule) {
			let expectedValue = rule.inputValue
				? rule.inputValue.toString().toLowerCase()
				: rule.inputValue;
