...
export class RepoAnalysisSettingInputProvider {
	...
	public async setInputControlValueFromRepoAnalysisResult(
		...
		if (this.selectedRepoAnalysisSettingIndex !== -1) {
					...
					if (value.length > 1) {
						...
						selectedValue = (
							await new ControlProvider().showQuickPick(
								...
								{
									placeHolder:
										inputControl.getInputDescriptor().name,
								},
							)
						).data;
					}
			...
			if (settingIndexMap.size === 0) {
				...
				if (
					...
					possibleValues = repoAnalysisValue.map((value) => ({
						label: value,
						data: value,
					}));
					if (possibleValues.length === 1) {
	...
	private async setValueIfNotPresentInRepoAnalysis(
		inputControl: InputControl,
		repoAnalysisSettingKey: string,
	): Promise<void> {
		...
		if (!value) {
			value = await new ControlProvider().showInputBox(
				repoAnalysisSettingKey,
				{
					...
					validateInput: (v) =>
						inputControl.triggerControlValueValidations(v),
				},
			);
		}
