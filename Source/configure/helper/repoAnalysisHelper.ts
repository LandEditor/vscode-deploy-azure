...
import {
	...
	RemoteServiceUrlHelper,
	ServiceFramework,
} from "./remoteServiceUrlHelper";
...
export class RepoAnalysisHelper {
	...
	public async getRepositoryAnalysis(
		sourceRepositoryDetails: GitRepositoryParameters,
		...
		repositoryAnalysisResponse.applicationSettingsList.forEach(
			(analysis) => {
				...
				if (
					Object.keys(SupportedLanguage).indexOf(
					...
					if (!!analysis.settings) {
						...
						if (!!analysis.buildTargetName) {
							...
							if (analysis.language === SupportedLanguage.NODE) {
								...
								if (
									...
									!!analysis.settings[
										RepoAnalysisConstants.GulpFilePath
									]
								) {
									applicationSettings.settings[
										RepoAnalysisConstants.GulpFilePath
									...
									analysis.buildTargetName ===
										RepoAnalysisConstants.Grunt &&
									!!analysis.settings[
										RepoAnalysisConstants.GruntFilePath
									]
								) {
									applicationSettings.settings[
										RepoAnalysisConstants.GruntFilePath
								...
								analysis.language === SupportedLanguage.PYTHON
							) {
								if (
									!!analysis.settings[
										RepoAnalysisConstants
									...
									applicationSettings.settings.pythonRequirementsFilePath =
										this.GetRelativePath(
											applicationSettings.settings
												.workingDirectory,
											analysis.settings[
						...
						if (!!analysis.deployTargetName) {
							...
							if (
								...
								applicationSettings.settings.azureFunctionsHostFilePath =
									analysis.settings[
	...
	private getClient(
		...
		if (serviceDefinition.serviceFramework === ServiceFramework.Vssf) {
			client = new PortalExtensionRepositoryAnalysisClient(
				...
				this.azureSession.credentials,
			);
		} else {
			client = new ModaRepositoryAnalysisClient(
				serviceDefinition.serviceUrl,
				this.githubPatToken,
			);
		}
