...
export async function mergingRepoAnalysisResults(
	...
	if (
		sourceRepository.repositoryProvider === RepositoryProvider.Github &&
...
export function getTargetKind(template: TemplateInfo): TargetKind {
	...
	if (template.attributes.deployTarget.toLowerCase().includes("webapp")) {
		if (
			...
			template.attributes.deployTarget.toLowerCase().includes("linux")
		) {
			targetKind = TargetKind.LinuxApp;
		} else if (
			template.attributes.deployTarget.toLowerCase().includes("windows")
		) {
...
export async function analyzeRepoAndListAppropriatePipeline(
	sourceRepository: GitRepositoryParameters,
	repoAnalysisParameters: RepositoryAnalysis,
	targetResource?: GenericResource,
): Promise<LocalPipelineTemplate[]> {
	let analysisResult = await mergingRepoAnalysisResults(
		sourceRepository,
		repoAnalysisParameters,
	);
		...
		switch (language) {
			case SupportedLanguage.DOCKER:
				if (
					...
					templateList[SupportedLanguage.DOCKER].length > 0
				) {
					templateResult = templateResult.concat(
						templateList[SupportedLanguage.DOCKER],
					);
				}
				break;
			...
			case SupportedLanguage.NODE:
				if (
					...
					templateList[SupportedLanguage.NODE].length > 0
				) {
					templateResult = templateResult.concat(
						templateList[SupportedLanguage.NODE],
					);
			...
			case SupportedLanguage.PYTHON:
				if (
					...
					templateList[SupportedLanguage.PYTHON].length > 0
				) {
					templateResult = templateResult.concat(
						templateList[SupportedLanguage.PYTHON],
					);
			...
			case SupportedLanguage.DOTNETCORE:
				if (
					...
					templateList[SupportedLanguage.DOTNETCORE].length > 0
				) {
					templateResult = templateResult.concat(
						templateList[SupportedLanguage.DOTNETCORE],
					);
			...
			case SupportedLanguage.NONE:
				if (
					...
					templateList[SupportedLanguage.NONE].length > 0
				) {
					templateResult = templateResult.concat(
						templateList[SupportedLanguage.NONE],
					);
	...
	templateResult =
		targetResource && !!targetResource.type
			? templateResult.filter(
					(template) =>
						!template.targetType ||
						template.targetType.toLowerCase() ===
	...
	templateResult =
		targetResource && !!targetResource.kind
			? templateResult.filter(
					(template) =>
						!template.targetKind ||
						template.targetKind.toLowerCase() ===
...
async function convertToPipelineTemplate(
	...
	switch (resourceType) {
		case TargetResourceType.AKS:
			await telemetryHelper.executeFunctionWithTimeTelemetry(async () => {
				filteredTemplates = await client.getTemplatesInfoByFilter(
					"docker",
					"Azure:AKS",
				);
...
export async function analyzeRepoAndListAppropriatePipeline2(
...
export function getPipelineTemplatesForAllWebAppKind(
	...
	return pipelineTemplates.filter((template) => {
		return (
			...
			template.targetType === TargetResourceType.WebApp &&
...
export function getDockerPort(
	...
	try {
		let dockerContent = fs.readFileSync(
		...
		telemetryHelper.logError(
			Layer,
			TracePoints.ReadingDockerFileFailed,
			err,
		);
	}
...
function isDotnetCoreRepo(files: string[]): boolean {
	return files.some((file) => {
		return (
			file.toLowerCase().endsWith("sln") ||
			file.toLowerCase().endsWith("csproj") ||
			file.toLowerCase().endsWith("fsproj")
		);
	});
}
	...
	"none": [
		{
			...
			path: path.join(
				...
				"configure/templates/azurePipelineTemplates/simpleWebApp.yml",
			),
			...
			parameters: [
				{
					"name": "webapp",
					"displayName":
						"Select the target Azure webapp to deploy your application",
		...
		},
		{
			...
			path: path.join(
				path.dirname(path.dirname(__dirname)),
				"configure/templates/azurePipelineTemplates/simpleLinuxWebApp.yml",
			),
			language: SupportedLanguage.NONE,
			targetType: TargetResourceType.WebApp,
			targetKind: TargetKind.LinuxApp,
	...
	"node": [
		{
			...
			path: path.join(
				path.dirname(path.dirname(__dirname)),
				"configure/templates/azurePipelineTemplates/nodejs.yml",
			),
		...
		},
		{
			...
			path: path.join(
				path.dirname(path.dirname(__dirname)),
				"configure/templates/azurePipelineTemplates/nodejsWithGulp.yml",
			),
			language: SupportedLanguage.NODE,
			targetType: TargetResourceType.WebApp,
			targetKind: TargetKind.WindowsApp,
		...
		},
		{
			...
			path: path.join(
				path.dirname(path.dirname(__dirname)),
				"configure/templates/azurePipelineTemplates/nodejsWithGrunt.yml",
			),
			language: SupportedLanguage.NODE,
			targetType: TargetResourceType.WebApp,
			targetKind: TargetKind.WindowsApp,
		...
		},
		{
			...
			path: path.join(
				path.dirname(path.dirname(__dirname)),
				"configure/templates/azurePipelineTemplates/nodejsWithAngular.yml",
			),
			language: SupportedLanguage.NODE,
			targetType: TargetResourceType.WebApp,
			targetKind: TargetKind.WindowsApp,
		...
		},
		{
			...
			path: path.join(
				path.dirname(path.dirname(__dirname)),
				"configure/templates/azurePipelineTemplates/nodejsWithWebpack.yml",
			),
			language: SupportedLanguage.NODE,
			targetType: TargetResourceType.WebApp,
			targetKind: TargetKind.WindowsApp,
		...
		},
		{
			...
			path: path.join(
				path.dirname(path.dirname(__dirname)),
				"configure/templates/azurePipelineTemplates/nodejsLinuxWebApp.yml",
			),
			language: SupportedLanguage.NODE,
			targetType: TargetResourceType.WebApp,
			targetKind: TargetKind.LinuxApp,
		...
		},
		{
			...
			path: path.join(
				path.dirname(path.dirname(__dirname)),
				"configure/templates/azurePipelineTemplates/nodejsWithGulpLinuxWebApp.yml",
			),
			language: SupportedLanguage.NODE,
			targetType: TargetResourceType.WebApp,
			targetKind: TargetKind.LinuxApp,
		...
		},
		{
			...
			path: path.join(
				path.dirname(path.dirname(__dirname)),
				"configure/templates/azurePipelineTemplates/nodejsWithGruntLinuxWebApp.yml",
			),
			language: SupportedLanguage.NODE,
			targetType: TargetResourceType.WebApp,
			targetKind: TargetKind.LinuxApp,
		...
		},
		{
			...
			path: path.join(
				path.dirname(path.dirname(__dirname)),
				"configure/templates/azurePipelineTemplates/nodejsWithAngularLinuxWebApp.yml",
			),
			language: SupportedLanguage.NODE,
			targetType: TargetResourceType.WebApp,
			targetKind: TargetKind.LinuxApp,
		...
		},
		{
			...
			path: path.join(
				path.dirname(path.dirname(__dirname)),
				"configure/templates/azurePipelineTemplates/nodejsWithWebpackLinuxWebApp.yml",
			),
			language: SupportedLanguage.NODE,
			targetType: TargetResourceType.WebApp,
			targetKind: TargetKind.LinuxApp,
	...
	"python": [
		{
			...
			path: path.join(
				path.dirname(path.dirname(__dirname)),
				"configure/templates/azurePipelineTemplates/pythonLinuxWebApp.yml",
			),
		...
		},
		{
			...
			path: path.join(
				path.dirname(path.dirname(__dirname)),
				"configure/templates/azurePipelineTemplates/pythonDjango.yml",
			),
			language: SupportedLanguage.PYTHON,
	...
	"dotnetcore": [
		{
			...
			path: path.join(
				path.dirname(path.dirname(__dirname)),
				"configure/templates/azurePipelineTemplates/dotnetcoreWindowsWebApp.yml",
			),
		...
		},
		{
			...
			path: path.join(
				path.dirname(path.dirname(__dirname)),
				"configure/templates/azurePipelineTemplates/dotnetcoreLinuxWebApp.yml",
			),
			language: SupportedLanguage.DOTNETCORE,
			targetType: TargetResourceType.WebApp,
			targetKind: TargetKind.LinuxApp,
	...
	"docker": [
		{
			...
			path: path.join(
				path.dirname(path.dirname(__dirname)),
				"configure/templates/azurePipelineTemplates/AksWithReuseACR.yml",
			),
			...
			parameters: [
				{
					"name": "aksCluster",
					"displayName":
						"Select Azure Kubernetes cluster to deploy your application",
					"type": TemplateParameterType.GenericAzureResource,
				...
				},
				{
					...
					"displayName":
						"Select Azure Container Registry to store docker image",
					"type": TemplateParameterType.GenericAzureResource,
	...
	"docker": [
		{
			...
			path: path.join(
				path.dirname(path.dirname(__dirname)),
				"configure/templates/githubWorkflowTemplates/AksWithReuseACR.yml",
			),
			language: SupportedLanguage.DOCKER,
			targetType: TargetResourceType.AKS,
			targetKind: null,
			...
			parameters: [
				...
				},
				{
					...
					"dataSourceId": "",
					"defaultValue":
						"{{#toLower}}{{#sanitizeString}}{{{inputs.aksCluster.name}}}{{/sanitizeString}}{{/toLower}}{{#tinyguid}}{{/tinyguid}}",
				},
			],
			assets: [
				{
	...
	"node": [
		{
			...
			path: path.join(
				path.dirname(path.dirname(__dirname)),
				"configure/templates/githubWorkflowTemplates/nodejsOnWindows.yml",
			),
			language: SupportedLanguage.NODE,
			targetType: TargetResourceType.WebApp,
			targetKind: TargetKind.WindowsApp,
		...
		},
		{
			...
			path: path.join(
				path.dirname(path.dirname(__dirname)),
				"configure/templates/githubWorkflowTemplates/nodejsOnLinux.yml",
			),
			language: SupportedLanguage.NODE,
			targetType: TargetResourceType.WebApp,
			targetKind: TargetKind.LinuxApp,
		...
		},
		{
			...
			path: path.join(
				path.dirname(path.dirname(__dirname)),
				"configure/templates/githubWorkflowTemplates/nodejsWithGulpOnWindowsWebApp.yml",
			),
			language: SupportedLanguage.NODE,
			targetType: TargetResourceType.WebApp,
			targetKind: TargetKind.WindowsApp,
		...
		},
		{
			...
			path: path.join(
				path.dirname(path.dirname(__dirname)),
				"configure/templates/githubWorkflowTemplates/nodejsWithGulpOnLinuxWebApp.yml",
			),
			language: SupportedLanguage.NODE,
			targetType: TargetResourceType.WebApp,
			targetKind: TargetKind.LinuxApp,
		...
		},
		{
			...
			path: path.join(
				path.dirname(path.dirname(__dirname)),
				"configure/templates/githubWorkflowTemplates/nodejsWithGruntOnWindowsWebApp.yml",
			),
			language: SupportedLanguage.NODE,
			targetType: TargetResourceType.WebApp,
			targetKind: TargetKind.WindowsApp,
		...
		},
		{
			...
			path: path.join(
				path.dirname(path.dirname(__dirname)),
				"configure/templates/githubWorkflowTemplates/nodejsWithGruntOnLinuxWebApp.yml",
			),
			language: SupportedLanguage.NODE,
			targetType: TargetResourceType.WebApp,
			targetKind: TargetKind.LinuxApp,
		...
		},
		{
			...
			path: path.join(
				path.dirname(path.dirname(__dirname)),
				"configure/templates/githubWorkflowTemplates/nodejsWithAngularOnWindowsWebApp.yml",
			),
			language: SupportedLanguage.NODE,
			targetType: TargetResourceType.WebApp,
			targetKind: TargetKind.WindowsApp,
		...
		},
		{
			...
			path: path.join(
				path.dirname(path.dirname(__dirname)),
				"configure/templates/githubWorkflowTemplates/nodejsWithAngularOnLinuxWebApp.yml",
			),
			language: SupportedLanguage.NODE,
			targetType: TargetResourceType.WebApp,
			targetKind: TargetKind.LinuxApp,
		...
		},
		{
			...
			path: path.join(
				path.dirname(path.dirname(__dirname)),
				"configure/templates/githubWorkflowTemplates/nodejsWithWebpackOnWindowsWebApp.yml",
			),
			language: SupportedLanguage.NODE,
			targetType: TargetResourceType.WebApp,
			targetKind: TargetKind.WindowsApp,
		...
		},
		{
			...
			path: path.join(
				path.dirname(path.dirname(__dirname)),
				"configure/templates/githubWorkflowTemplates/nodejsWithWebpackOnLinuxWebApp.yml",
			),
			language: SupportedLanguage.NODE,
			targetType: TargetResourceType.WebApp,
			targetKind: TargetKind.LinuxApp,
	...
	"none": [
		{
			...
			path: path.join(
				path.dirname(path.dirname(__dirname)),
				"configure/templates/githubWorkflowTemplates/simpleWebApp.yml",
			),
			language: SupportedLanguage.NONE,
			targetType: TargetResourceType.WebApp,
			targetKind: TargetKind.WindowsApp,
	...
	"python": [
		{
			...
			path: path.join(
				path.dirname(path.dirname(__dirname)),
				"configure/templates/githubWorkflowTemplates/pythonLinuxWebApp.yml",
			),
			language: SupportedLanguage.PYTHON,
			targetType: TargetResourceType.WebApp,
			targetKind: TargetKind.LinuxApp,
...
const azurePipelineTargetBasedTemplates: {
	[key in AzureTarget]: LocalPipelineTemplate[];
} = {
	"Microsoft.Web/sites-functionapp": [
		{
			...
			path: path.join(
				path.dirname(path.dirname(__dirname)),
				"configure/templates/azurePipelineTemplates/nodejsWindowsFunctionApp.yml",
			),
			language: SupportedLanguage.NODE,
			targetType: TargetResourceType.WebApp,
			targetKind: TargetKind.FunctionApp,
			...
			parameters: [
				{
					"name": "webapp",
					"displayName":
						"Select the target Azure Function to deploy your application",
					"type": TemplateParameterType.GenericAzureResource,
		...
		},
		{
			...
			path: path.join(
				path.dirname(path.dirname(__dirname)),
				"configure/templates/azurePipelineTemplates/nodejsLinuxFunctionApp.yml",
			),
			language: SupportedLanguage.NODE,
			targetType: TargetResourceType.WebApp,
			targetKind: TargetKind.FunctionAppLinux,
		...
		},
		{
			...
			path: path.join(
				path.dirname(path.dirname(__dirname)),
				"configure/templates/azurePipelineTemplates/dotnetcoreWindowsFunctionApp.yml",
			),
			language: SupportedLanguage.DOTNETCORE,
			targetType: TargetResourceType.WebApp,
			targetKind: TargetKind.FunctionApp,
		...
		},
		{
			...
			path: path.join(
				path.dirname(path.dirname(__dirname)),
				"configure/templates/azurePipelineTemplates/dotnetcoreLinuxFunctionApp.yml",
			),
			language: SupportedLanguage.DOTNETCORE,
			targetType: TargetResourceType.WebApp,
			targetKind: TargetKind.FunctionAppLinux,
		...
		},
		{
			...
			path: path.join(
				path.dirname(path.dirname(__dirname)),
				"configure/templates/azurePipelineTemplates/pythonLinuxFunctionApp.yml",
			),
			language: SupportedLanguage.PYTHON,
			targetType: TargetResourceType.WebApp,
			targetKind: TargetKind.FunctionAppLinux,
...
const githubWorkflowTargetBasedTemplates: {
	[key in AzureTarget]: LocalPipelineTemplate[];
} = {
	"Microsoft.Web/sites-functionapp": [
		{
			...
			path: path.join(
				path.dirname(path.dirname(__dirname)),
				"configure/templates/githubWorkflowTemplates/nodejsWindowsFunctionApp.yml",
			),
			language: SupportedLanguage.NODE,
			targetType: TargetResourceType.WebApp,
			targetKind: TargetKind.FunctionApp,
		...
		},
		{
			...
			path: path.join(
				path.dirname(path.dirname(__dirname)),
				"configure/templates/githubWorkflowTemplates/nodejsLinuxFunctionApp.yml",
			),
			language: SupportedLanguage.NODE,
			targetType: TargetResourceType.WebApp,
			targetKind: TargetKind.FunctionAppLinux,
		...
		},
		{
			...
			path: path.join(
				path.dirname(path.dirname(__dirname)),
				"configure/templates/githubWorkflowTemplates/pythonLinuxFunctionApp.yml",
			),
			language: SupportedLanguage.PYTHON,
			targetType: TargetResourceType.WebApp,
			targetKind: TargetKind.FunctionAppLinux,
			...
			parameters: [
				{
					...
					"type": TemplateParameterType.GenericAzureResource,
					"dataSourceId":
						PreDefinedDataSourceIds.LinuxContainerFunctionApp,
				},
			],
			assets: [
				{
