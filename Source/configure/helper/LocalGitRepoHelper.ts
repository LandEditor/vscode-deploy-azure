...
export class LocalGitRepoHelper {
	public static GetHelperInstance(
		...
		telemetryHelper.setTelemetry(
			TelemetryKeys.GitFolderExists,
			gitFolderExists.toString(),
		);

		return repoService;
	}

	public static async GetAvailableFileName(
		fileName: string,
		repoPath: string,
	): Promise<string> {
	...
	private static getIncreamentalFileName(
		fileName: string,
		count: number,
	): string {
		return fileName
			.substr(0, fileName.indexOf("."))
			.concat(` (${count})`, fileName.substr(fileName.indexOf(".")));
	}
	...
	public async getUsername(): Promise<string> {
		let username = await this.gitReference.raw(["config", "user.name"]);
	...
	public async getGitRemoteUrl(remoteName: string): Promise<string | void> {
		...
		if (AzureDevOpsHelper.isAzureReposUrl(<string>remoteUrl)) {
			remoteUrl = AzureDevOpsHelper.getFormattedRemoteUrl(
				<string>remoteUrl,
			);
	 ...
	 */
	public async commitAndPushPipelineFile(
		files: string[],
		repositoryDetails: GitRepositoryParameters,
		...
		try {
			...
			if (repositoryDetails.remoteName && repositoryDetails.branch) {
				await this.gitReference.push(
					repositoryDetails.remoteName,
	...
	public async createAndDisplayManifestFile(
		manifestFile: string,
		...
		inputs.pipelineConfiguration.assets[manifestFile] = path
			.relative(await this.getGitRootDirectory(), manifestFilePath)
			.split(path.sep)
			.join("/");
		filesToCommit.push(manifestFilePath);
		await this.addContentToFile(
			await templateHelper.renderContent(
				manifestPath + manifestFile + ".yml",
				mustacheContext,
			),
			manifestFilePath,
		);
