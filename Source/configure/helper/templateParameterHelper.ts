...
import {
	ApiVersions,
	AzureResourceClient,
} from "../clients/azure/azureResourceClient";
...
export class TemplateParameterHelper {
	public static getParameterForTargetResourceType(
		parameters: TemplateParameter[],
		targetResourceType: TargetResourceType,
		targetResourceKind?: TargetKind,
	): TemplateParameter {
		let dataSourceIdForResourceType =
			TemplateParameterHelper.convertAzureResourceToDataSourceId(
				targetResourceType,
				targetResourceKind,
			);
	...
	public static getParameterValueForTargetResourceType(
		...
		if (!parameterValue) {
			throw utils.format(
				...
				resourceTemplateParameter.name,
			);
		}
	...
	public static getMatchingAzureResourceTemplateParameter(
		...
		let matchedParam = templateParameters.find((templateParameter) => {
			return (
				...
				resourceTargetType.toLowerCase()
			);
		});
	...
	private static convertAzureResourceToDataSourceId(
		...
		if (!!parameter) {
			switch (parameter.type) {
				...
				default:
					throw new Error(
						utils.format(
							Messages.parameterOfTypeNotSupported,
							parameter.type,
						),
					);
			}
	...
	private async getAzureResourceParameter(
		parameter: TemplateParameter,
		inputs: WizardInputs,
	): Promise<void> {
		...
		let azureResourceClient = new AzureResourceClient(
		...
		if (!!parameter) {
			switch (parameter.dataSourceId) {
				...
				case PreDefinedDataSourceIds.AKS:
					let azureResourceListPromise = azureResourceClient
						.getResourceList(parameter.dataSourceId, true)
						.then((list) =>
							list.map((x) => {
					...
					while (1) {
						let selectedResource =
							await controlProvider.showQuickPick(
								...
								utils.format(
									TelemetryKeys.pickListCount,
									parameter.dataSourceId,
								),
							);

						let detailedResource =
							await this.tryGetSelectedResourceById(
								selectedResource.data.id,
								azureResourceClient,
								ApiVersions.get(
									parameter.dataSourceId ===
										PreDefinedDataSourceIds.ACR
						...
						if (
							...
							if (
								detailedResource.properties.adminUserEnabled ===
								false
							) {
								controlProvider.showErrorMessage(
							...
							if (detailedResource.properties.addonProfiles) {
								let addonProfiles = JSON.parse(
									JSON.stringify(
										detailedResource.properties
											.addonProfiles,
									).toLowerCase(),
								);
				...
				case PreDefinedDataSourceIds.LinuxFunctionApp:
					...
					let matchingPipelineTemplates =
						templateHelper.getPipelineTemplatesForAllWebAppKind(
							...
							selectedPipelineTemplate.label,
							selectedPipelineTemplate.language,
							selectedPipelineTemplate.targetKind,
						);
						...
						inputs.pipelineConfiguration.template =
							matchingPipelineTemplates.find(
								(template) =>
									template.targetKind ===
									<TargetKind>(
										inputs.targetResource.resource.kind
									),
							);
					}
					break;
				...
				default:
					throw new Error(
						utils.format(
							Messages.parameterWithDataSourceOfTypeNotSupported,
							parameter.dataSourceId,
						),
					);
			}
	...
	private async setSubscription(inputs: WizardInputs): Promise<void> {
		...
		let selectedSubscription: QuickPickItemWithData =
			await new ControlProvider().showQuickPick(
	...
	private async tryGetSelectedResourceById(
		selectedResourceId: string,
		azureResourceClient: AzureResourceClient,
		getResourceApiVersion?: string,
		...
		try {
			...
			if (getResourceApiVersion) {
				...
				detailedResource =
					await azureResourceClient.getResource(selectedResourceId);
			}
	...
	private async getStringParameter(
		parameter: TemplateParameter,
		inputs: WizardInputs,
	): Promise<void> {
		let controlProvider = new ControlProvider();
		...
		if (!parameter.dataSourceId) {
			if (parameter.defaultValue) {
				...
				inputs.pipelineConfiguration.params[parameter.name] =
					await controlProvider.showInputBox(parameter.name, {
						placeHolder: parameter.displayName,
					});
			}
		} else {
