...
export class AssetHandler {
	...
	public async createAssets(
		...
		if (
			inputs.pipelineConfiguration.template.label ===
			"Containerized application to AKS"
		) {
						...
						await vscode.window.withProgress(
							{
								location: vscode.ProgressLocation.Notification,
								title: utils.format(
									Messages.creatingKubernetesConnection,
							...
							async () => {
								try {
									...
									return await createAsset(
										assetName,
										asset.type,
										SodiumLibHelper.decodeFromBase64(
											JSON.stringify(
						...
						await vscode.window.withProgress(
							{
								location: vscode.ProgressLocation.Notification,
								title: utils.format(
									Messages.creatingContainerRegistryConnection,
							...
							async () => {
								try {
									...
									if (
										...
										let password =
											!!registryCreds.passwords &&
											registryCreds.passwords.length > 0
