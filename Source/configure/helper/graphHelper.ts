...
export class GraphHelper {
	...
	public static generateAadApplicationName(
		...
		if (
			...
			accountName.length > spnLengthAllowed / 2 &&
			accountName.length + projectName.length > spnLengthAllowed
		) {
			accountName = accountName.substr(
				0,
				spnLengthAllowed - projectName.length,
			);
		} else if (
			projectName.length > spnLengthAllowed / 2 &&
			accountName.length + projectName.length > spnLengthAllowed
	...
	private static async getGraphToken(
		...
		return this.getResourceTokenFromRefreshToken(
			...
			session.tenantId,
			(<any>session.credentials).clientId,
			session.environment.activeDirectoryGraphResourceId,
		);
	}
	...
	private static async getResourceTokenFromRefreshToken(
				...
				(err, tokenResponse) => {
					if (err) {
						reject(
							new Error(
								util.format(
									Messages.acquireTokenFromRefreshTokenFailed,
									err.message,
								),
							),
						);
	...
	private static async createAadApp(
		...
		return graphClient
			.sendRequest<any>(<UrlBasedRequestPrepareOptions>{
	...
	private static async createSpn(
		graphClient: RestClient,
		appId: string,
		tenantId: string,
	): Promise<any> {
	...
	private static async createRoleAssignment(
