...
export async function generateGitHubRepository(
	orgName: string,
	localPath: string,
	githubClient: GithubClient,
	isUserAccount: boolean = false,
): Promise<GitHubRepo> {
	let repoName = localPath
		.substring(localPath.lastIndexOf("\\") + 1)
		.substring(localPath.lastIndexOf("/") + 1);
	let repoDetails = (await githubClient.createGithubRepo(
		orgName,
		repoName,
		isUserAccount,
	)) as GitHubRepo;
	...
	return await githubClient.createGithubRepo(
		orgName,
		repoName + "_" + uuid().substr(0, 5),
		isUserAccount,
	);
}

export function generateDevOpsOrganizationName(
	userName: string,
