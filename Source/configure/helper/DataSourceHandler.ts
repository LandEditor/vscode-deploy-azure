...
export class DataSourceHandler {
	public getResultFromPreDefinedDataSourceId(
		dataSourceId: string,
		inputs: WizardInputs,
	): Promise<any> {
		switch (dataSourceId) {
			case PreDefinedDataSourceIds.AKS:
				...
				let kubeConfig =
					restClient.sendRequestWithHttpOperationResponse({
						url:
							inputs.azureSession.environment.portalUrl +
							`/subscriptions/${parsedResourceId.subscriptionId}/resourceGroups/${parsedResourceId.resourceGroup}/providers/Microsoft.ContainerService/managedClusters/${parsedResourceId.resourceName}/listClusterAdminCredential?api-version=2020-01-01`,
						method: "POST",
