...
export class ServiceConnectionHelper {
	...
	public constructor(
		...
		this.serviceConnectionClient = new ServiceConnectionClient(
			organizationName,
			projectName,
			azureDevOpsClient,
		);
	...
	public async createGitHubServiceConnection(
		...
		let response =
			await this.serviceConnectionClient.createGitHubServiceConnection(
				name,
				gitHubPat,
			);
	...
	public async createAzureSPNServiceConnection(
		name: string,
		tenantId: string,
		...
		let response =
			await this.serviceConnectionClient.createAzureSPNServiceConnection(
				name,
				tenantId,
				subscriptionId,
				scope,
	...
	public async createAzurePublishProfileServiceConnection(
		name: string,
		tenantId: string,
		resourceId: string,
		...
		let response =
			await this.serviceConnectionClient.createAzurePublishProfileServiceConnection(
				name,
				tenantId,
				resourceId,
				publishProfile,
	...
	public async createKubeConfigServiceConnection(
		name: string,
		kubeConfig: string,
		apiServerAddress: string,
	): Promise<string> {
		let response =
			await this.serviceConnectionClient.createKubernetesServiceConnectionWithKubeConfig(
				name,
				kubeConfig,
				apiServerAddress,
			);
	...
	public async createContainerRegistryServiceConnection(
		name: string,
		registryUrl: string,
		...
		let response =
			await this.serviceConnectionClient.createContainerRegistryServiceConnection(
				name,
				registryUrl,
				registryUsername,
				registryPassword,
	...
	private async waitForEndpointToBeReady(endpointId: string): Promise<void> {
		...
		while (1) {
			...
			if (
				!(retryCount < 30) ||
				operationStatus.state.toLowerCase() === "failed"
			) {
				throw Error(
					util.format(
						Messages.unableToCreateAzureServiceConnection,
	...
	private async waitForGitHubEndpointToBeReady(
		endpointId: string,
	): Promise<void> {
		let retryCount = 1;
		while (1) {
			...
			if (!(retryCount < 40)) {
				throw Error(
					util.format(
						Messages.unableToCreateGitHubServiceConnection,
						isReady,
					),
				);
			}
